...
import { getClosestPointBylineLine, polyPoint } from 'el/utils/collision';
import { createBezier } from "el/utils/bezier";
...
export class MeshTransform {
    ...
    makeVector(row, col) {
        ...
        this.makePathVectorMap(bottom[0][0], bottom[0][1], bottom[1][0], bottom[1][1]);
    ...
    _releaseVector(vector, path) {
        const {
            top, left, right, bottom, pathVerties
        } = vector;

        const topKey = this.makePathVectorKey(top[0][0], top[0][1], top[1][0], top[1][1]);
        const leftKey = this.makePathVectorKey(left[0][0], left[0][1], left[1][0], left[1][1]);
        const rightKey = this.makePathVectorKey(right[0][0], right[0][1], right[1][0], right[1][1]);
        const bottomKey = this.makePathVectorKey(bottom[0][0], bottom[0][1], bottom[1][0], bottom[1][1]);
        ...
        pathVerties.forEach((pv, index) => {
            const [ x, y ] = this.calculateCubicInterpolate(pv, topCurveFunction, leftCurveFunction, rightCurveFunction, bottomCurveFunction);
    ...
    calculateCubicInterpolate(pv, topCurveFunction, leftCurveFunction, rightCurveFunction, bottomCurveFunction) {
        ...
        return getClosestPointBylineLine(
            vector1[0][0], vector1[0][1], vector1[1][0], vector1[1][1],
