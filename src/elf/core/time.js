import { isNumber, isString, isFunction } from "sapa";
...
export function makeTimer(opt) {
  ...
  const tick = (now) => {
    ...
    var elapsed =
      calculateForDirection(timer.elapsed / timer.duration) * timer.duration;
  ...
  const play = (opt = {}) => {
    ...
    if (isNumber(opt.iterationCount))
      timer.iterationCount = opt.iterationCount || Number.MAX_SAFE_INTEGER;
