...
function invertTargetPoint(arr, isInvert = false) {
  ...
  const [source, target, axis, dist, newTarget, sourceVerties, targetVerties] =
    arr;
...
export function makeGuidePoint(sourceVerties, targetVerties) {
  ...
  if (
    rightVerties[0][1] <= leftCenter[1] &&
    leftCenter[1] <= rightVerties[3][1]
  ) {
    pointList.push(getTargetPointX(leftCenter, rightCenter, "x", null));
  } else if (rightVerties[0][1] <= leftVerties[1][1]) {
    if (
      rightVerties[3][0] <= leftVerties[1][0] &&
      leftVerties[1][0] <= rightVerties[2][0]
    ) {
      pointList.push(
        getTargetPointY(
          rightVerties[3],
          [rightVerties[3][0], leftVerties[1][1], leftVerties[1][2]],
          "y",
          null
        )
      );
      ...
      pointList.push(
        getTargetPointX(leftVerties[1], rightCenter, "x", rightVerties[3])
      );
    }
  } else if (
    leftVerties[3][0] <= rightVerties[0][0] &&
    rightVerties[0][0] <= leftVerties[2][0]
  ) {
    pointList.push(getTargetPointY(leftVerties[3], rightVerties[0], "y"));
  } else if (
    rightVerties[3][0] <= leftVerties[0][0] &&
    leftVerties[0][0] <= rightVerties[2][0]
  ) {
    pointList.push(
      getTargetPointY(
        [rightVerties[0][0], leftVerties[3][1], leftVerties[3][2]],
        rightVerties[0],
        "y"
      )
    );
    ...
    pointList.push(
      getTargetPointX(leftVerties[2], rightCenter, "x", rightVerties[0])
    );
  }
  ...
  return invertTargetPoint(
    [...pointList[0], sourceVerties, targetVerties],
    hasInvert
  );
}
...
export function cubicBezier(x1, y1, x2, y2) {
  ...
  return function (progress) {
    ...
    var y =
      C2.y * bezierCalc.B2(progress) +
      C3.y * bezierCalc.B3(progress) +
      bezierCalc.B4(progress);
...
export function getGradientLine(angle, box) {
  let length =
    Math.abs(box.width * Math.sin(angle)) +
    Math.abs(box.height * Math.cos(angle));
  let center = {
    x: box.x + box.width / 2,
