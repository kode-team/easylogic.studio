...
const CSS_FUNC_REGEXP =
  /(([-]?[\d.]+)(px|pt|fr|r?em|deg|vh|vw|m?s|%|g?rad|turn)?)|#(?:[\da-f]{8})|(#(?:[\da-f]{3}){1,2}|([a-z_-]+)\([^()]+\)|([a-z_-]+))|(,)/gi;
const CSS_LENGTH_REGEXP =
  /^[-]?([\d.]+)(px|pt|fr|r?em|deg|vh|vw|m?s|%|g?rad|turn)?$/gi;
...
const CSS_FUNC_PARSER_MAP = {
  ...
  hex: (item) => ({
    ...
    ...Color.parse(item.matchedString),
  }),
  rgb: (item) => ({
    funcType: FuncType.COLOR,
    ...Color.parse(item.matchedString),
...
export function parseValue(
    ...
    return result.some((it) => {
      if (it.parsed && isArray(it.parsed)) {
        return it.parsed.some((parsedIt) => {
          if (
            parsedIt.startIndex === startIndex &&
            parsedIt.endIndex === endIndex &&
            matchedString === parsedIt.matchedString
          ) {
            return true;
          }
      ...
      if (
        ...
        matchedString === it.matchedString
      ) {
        return true;
      }
  ...
  for (var i = 0, len = matches.length; i < len; i++) {
    ...
    const isContinue = checkParsedResult(
      item.startIndex,
      item.endIndex,
      item.matchedString
    );
    ...
    if (CSS_FUNC_PARSER_MAP[parsedFunc]) {
      customFunctionCallback =
        CSS_FUNC_PARSER_MAP[parsedFunc] ||
        CSS_FUNC_PARSER_MAP[item.matchedString];
    } else if (customFuncMap[parsedFunc] || customFuncMap[item.matchedString]) {
      customFunctionCallback =
        customFuncMap[parsedFunc] || customFuncMap[item.matchedString];
    }

    if (customFunctionCallback) {
      const parsed = customFunctionCallback.call(
        null,
        item,
        str,
        funcStartCharacter,
