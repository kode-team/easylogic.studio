...
export default class PathGenerator {
  ...
  getCacheSegmentKey(segmentKey, index) {
    ...
    if (!this.cachedSegmentKeys[segmentKey][index]) {
      this.cachedSegmentKeys[segmentKey][index] = this.makeSegmentKey({
        key: segmentKey,
        index,
      });
    }
  ...
  transformMat4(transformMatrix) {
    this.transformPoints.forEach((p, index) => {
      ...
      Object.assign(
        ...
        this.commitTransformMatrix(p.startPoint, transformMatrix)
      );
      Object.assign(
        realPoint.endPoint,
        this.commitTransformMatrix(p.endPoint, transformMatrix)
      );
      Object.assign(
        realPoint.reversePoint,
        this.commitTransformMatrix(p.reversePoint, transformMatrix)
      );
    });
  }
  ...
  setCachePoint(index, segmentKey) {
    ...
    state.connectedPointList = clone(
      Point.getConnectedPointList(this.points, this.selectedIndex)
    );
    ...
    state.isCurveSegment =
      state.segment.curve && state.segmentKey != "startPoint";
  ...
  convertToCurve(index) {
    ...
    if (point.curve) {
      ...
      if (point.command === "M") {
        var lastPoint = Point.getPrevPoint(this.points, point.index);
      ...
      if (nextPoint && nextPoint.index < index && nextPoint.command === "M") {
        ...
        point.reversePoint = Point.getReversePoint(
          point.startPoint,
          point.endPoint
        );
  ...
  moveCurveSegment(segmentKey, dx, dy) {
    ...
    var targetSegmentKey =
      segmentKey === "endPoint" ? "reversePoint" : "endPoint";

    state.segment[targetSegmentKey] = Point.getReversePoint(
      state.segment.startPoint,
      state.segment[segmentKey]
    );
  }
  ...
  calculateSnap(segmentKey, dx, dy, dist = 1) {
    ...
    var { point: snapPointX, distanceValue: dx } = calculateSnapPoint(
      cachedPoints,
      "x",
      realX,
      dx,
    ...
    var { point: snapPointY, distanceValue: dy } = calculateSnapPoint(
      cachedPoints,
      "y",
      realY,
      dy,
  ...
  get selectedGroupIndexList() {
    ...
    if (
      this.selectedPointList.length === 0 &&
  ...
  removeSelectedSegment() {
    ...
    const newPoints = Point.recoverPoints(
      pointGroup.map((points) => {
        return points
          .filter((p) => !p.startPoint.removed)
          .map((p) => {
            if (p.endPoint.removed) {
              p.endPoint = clone(p.startPoint);
            }
            if (p.reversePoint.removed) {
              p.reversePoint = clone(p.startPoint);
            }
  ...
  makeTriangleDistancePointGuide(first, second) {
    ...
    // right bottom
    if (
      first.startPoint.x < second.startPoint.x &&
      first.startPoint.y < second.startPoint.y
    ) {
      this.segmentManager
      ...
      var dist = 20;
      var { x, y } = getXYInCircle(0, dist, centerX, centerY);
      // var { x: tx, y: ty } = getXYInCircle(angle / 2, dist, centerX, centerY);

      var last = getXYInCircle(angle, dist, centerX, centerY);

      this.segmentManager.addDistanceAngle(
        last,
        dist,
        dist,
        angle,
  ...
  makeStartPointGuide(prevPoint, current, nextPoint, index) {
    ...
    if (current.curve === false) {
      this.segmentManager.addPoint(
        ...
        index,
        "startPoint",
        this.isSelectedSegment("startPoint", index)
      );
    } else {
      this.segmentManager
        .addPoint(
      ...
      if (Point.isEqual(current.startPoint, current.endPoint) === false) {
        this.segmentManager.addCurvePoint(
          current.endPoint,
          index,
          "endPoint",
          this.isSelectedSegment("endPoint", index)
        );
      }
    }
  }

  makeMiddlePointGuideSegment(
    prevPoint,
    current,
    nextPoint,
    index,
    ...
    if (current.curve === false) {
      ...
      if (prevPoint.curve === false) {
        ...
        if (isSiblingSelected === false) {
          ...
          if (
            Point.isEqual(prevPoint.startPoint, prevPoint.endPoint) === false
          ) {
            mng.addCurvePoint(
              prevPoint.endPoint,
              prevPoint.index,
              "endPoint",
              this.isSelectedSegment("endPoint", prevPoint.index)
            );
          }
        }
      }
      ...
      if (prevPoint.curve === false) {
        if (isSiblingSelected === false) {
          ...
          if (Point.isEqual(current.reversePoint, current.startPoint)) {
            ...
            if (
              Point.isEqual(current.startPoint, current.reversePoint) === false
            ) {
              mng.addCurvePoint(
                current.reversePoint,
                index,
                "reversePoint",
                this.isSelectedSegment("reversePoint", index)
              );
            }
          }
        }
      ...
      if (prevPoint.curve === false) {
        if (
          (this.checkInViewport(prevPoint.startPoint) ||
            this.checkInViewport(current.startPoint)) === false
        ) {
          return;
        }
  ...
  makePointGuide(points) {
    for (var index = 0, len = points.length; index < len; index++) {
      ...
      if (current.command === "M") {
        ...
        this.makeMiddlePointGuideSplitLine(
          prevPoint,
          current,
          nextPoint,
          index,
        ...
        this.makeMiddlePointGuideSegment(
          prevPoint,
          current,
          nextPoint,
          index,
  ...
  makeMovePositionGuide() {
    ...
    if (moveXY) {
      ...
      var { snapPointList: movePointSnapPointList, moveXY: newMoveXY } =
   ...
   */
  makePathArea() {
    const pathList = this.splitedGroupList.map(
      ({ startPointIndex, points }, groupIndex) => {
            ...
            <g>
               ${pathList
                 .map((it) => {
                   ...
                   const [x, y] = center;
                   const selected =
                     this.state.selectedGroupIndex === it.groupIndex;
                   return /*html*/ `
                        ...
                        ${
                          pathCount > 1 &&
                          /*html*/ `
                            <text class="path-area-text" x="${x}" y="${y}" >${
                            it.groupIndex + 1
                          }</text>
                        `
                        }
