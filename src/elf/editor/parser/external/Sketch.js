...
export const SketchUtil = {
      ...
      fills.forEach((fillObject) => {
        switch (fillObject.fillType) {
          case Sketch.Style.FillType.Color:
            results["background-color"] = SketchUtil.makeColor(
              fillObject.color
            );
            break;
  ...
  convertAxis(axis, multi = 100) {
    ...
    const [x, y] = axis
      .substring(1, axis.length - 2)
      .split(",")
      .map((it) => Length.percent(it.trim()).mul(multi));
    ...
    switch (fillObject.gradient.gradientType) {
      ...
      case Sketch.Style.GradientType.Radial:
        results = new RadialGradient({
          radialType:
            fillObject.gradient.aspectRatio === 1 ? "circle" : "ellipse",
  ...
  makePath(obj) {
    ...
    points.forEach((p) => {
      switch (p.curveMode) {
        case Sketch.CurvePoint.PointType.Undefined:
          break;
        ...
        case Sketch.CurvePoint.PointType.Mirrored:
          break;
        // C
        case Sketch.CurvePoint.PointType.Asymmetric:
          break;
        // 비대칭
  ...
  addLayer(parentInstance, parentObject) {
  ...
  parse(sketchData) {
    ...
    Object.keys(sketchData)
      .filter((it) => it.startsWith("pages/"))
      .forEach((key) => {
        ...
        artboards.forEach((artboard) => {
          ...
          var artboardObject = project.add(
            new ArtBoard({
              name: artboard.name,
