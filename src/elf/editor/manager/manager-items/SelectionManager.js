...
export class SelectionManager {
  ...
  isSameIds(newIds) {
    ...
    if (
      this.ids.filter((id) => newIds.includes(id)).length === this.ids.length
    ) {
      return true;
    }
  ...
  // group 을 선택할 때 사용한다.
  selectByGroup(...ids) {
    var list = this.modelManager
      .searchItemsById(this.filterIds(ids || []))
      .filter((it) => {
  ...
  select(...ids) {
    var list = this.modelManager
      .searchItemsById(this.filterIds(ids || []))
      .filter((it) => !it.lock && it.isAbsolute);
    ...
    this.each((instance) => {
      if (instance.is("artboard") === false) {
        ...
        if (instance.artboard) {
          ...
          const isInArtboard = polyPoint(
            localArtboardVerties,
            this.pos[0],
            this.pos[1]
          );
        ...
        if (selectedArtBoard) {
          ...
          if (
            selectedArtBoard.item !== instance.artboard &&
            selectedArtBoard.item.hasLayout() === false
          ) {
  ...
  packByValue(valueObject, ids = null) {
    ...
    localItems.forEach((item) => {
      ...
      Object.keys(valueObject).forEach((key) => {
        data[item.id][key] = isFunction(valueObject[key])
          ? valueObject[key].call(valueObject, item)
          : valueObject[key];
      });
    });
  ...
  setHoverId(id) {
    ...
    if (!id || this.itemKeys[id]) {
      ...
      this.hoverId = id;
      this.hoverItems = this.itemsByIds([id]).filter((it) =>
        it.isNot("artboard")
      );
