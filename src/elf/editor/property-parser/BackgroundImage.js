...
export class BackgroundImage extends PropertyItem {
  ...
  recoverOffset(newX, newY, contentBox, dx = 0, dy = 0, options = {}) {
    ...
    const newHeight = options.shiftKey
      ? newWidth
      : Math.floor(height.toPx(contentBox.height).value + dy);
  ...
  getOffset(contentBox) {
    ...
    return {
      x:
        contentBox.x +
        (x.isPercent()
          ? (contentBox.width - newWidth) * (x.value / 100)
          : newX),
      y:
        contentBox.y +
        (y.isPercent()
          ? (contentBox.height - newHeight) * (y.value / 100)
          : newY),
  ...
  toBackgroundVisibilityCSS() {
    ...
    return {
      "background-visibility":
        json.visibility === VisibilityType.HIDDEN
  ...
  static parseStyle(style) {
    ...
    if (style["background-visibility"]) {
      style["background-visibility"]
        .split(",")
        .map((it) => it.trim())
        .forEach((it, index) => {
          if (backgroundImages[index]) {
            backgroundImages[index].visibility =
              it === VisibilityType.HIDDEN
                ? VisibilityType.HIDDEN
                : VisibilityType.VISIBLE;
          }
        });
  ...
  static join(list) {
    return CSS_TO_STRING(
      BackgroundImage.toProperty(list.map((it) => BackgroundImage.parse(it)))
    );
  }
