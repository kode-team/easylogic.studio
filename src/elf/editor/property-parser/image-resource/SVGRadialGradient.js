...
export class SVGRadialGradient extends SVGGradient {
  convert(json) {
    json.spreadMethod = SpreadMethodList.includes(json.spreadMethod)
      ? json.spreadMethod
      : SpreadMethodType.PAD;
  ...
  toString() {
    ...
    var opt = [radialType, x1, y1, x2, y2, x3, y3, spreadMethod]
      .join(" ")
      .trim();
  ...
  getGradientAngle(contentBox = {}) {
    ...
    const angle =
      calculateAngle360(newX2.value - newX1.value, newY2.value - newY1.value) +
      180;
  ...
  toSVGString(id, contentBox = {}) {
    ...
    mat3.multiply(
      view,
      view,
      mat3.fromTranslation([], [newX1.value, newY1.value])
    );
...
<radialGradient ${OBJECT_TO_PROPERTY({
    ...
    ${SVGRadialGradient.makeColorStepList(this.colorsteps)
      .map(
        (it) => /*html*/ `
