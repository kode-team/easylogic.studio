  ...
  for (var i = 0, len = totalLength; i < len; i++) {
    ...
    if (startObject && !endObject) {
      ...
      list.push({
        ...
        blendMode: makeInterpolateIdentity(
          layer,
          property,
          startObject.blendMode
        ),
      ...
      // 처음도 있고 끝도 있을 때
      list.push({
        image: makeInterpolateImageResource(
          layer,
          property,
          startObject.image,
          endObject.image
        ),
        size: makeInterpolateString(
          layer,
          property,
          startObject.size,
          endObject.size
        ),
        width: makeInterpolateLength(
          layer,
          property,
          startObject.width,
          endObject.width,
        ...
        blendMode: makeInterpolateString(
          layer,
          property,
          startObject.blendMode,
          endObject.blendMode
        ),
        repeat: makeInterpolateString(
          layer,
          property,
          startObject.repeat,
          endObject.repeat
