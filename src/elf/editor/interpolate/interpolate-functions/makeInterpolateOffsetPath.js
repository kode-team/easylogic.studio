...
export function makeInterpolateOffsetPath(
  ...
  var [id, distance, rotateStatus, rotate] = startValue
    .split(",")
    .map((it) => it.trim());

  var startObject = {
    id,
    distance: Length.parse(distance || "0%"),
    rotateStatus: rotateStatus || "auto",
    rotate: Length.parse(rotate || "0deg"),
  };
  ...
  return (rate, t, timing) => {
    ...
    if (startObject.rotateStatus === "element") {
      ...
      var newAngle = Length.deg(
        innerInterpolateAngle(startObject.rotateStatus, angle)
      );
