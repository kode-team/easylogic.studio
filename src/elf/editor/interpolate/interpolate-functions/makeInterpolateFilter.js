...
export function makeInterpolateFilter(layer, property, startValue, endValue) {
  ...
  for (var i = 0, len = totalLength; i < len; i++) {
    ...
    if (startObject && !endObject) {
      ...
      if (
        startObject.type != endObject.type ||
        startObject.type === "svg" ||
        endObject.type === "svg"
      ) {
        list.push(
          makeInterpolateBoolean(layer, property, startObject, endObject)
        );
      } else {
        list.push(
          makeInterpolateFilterItem(layer, property, startObject, endObject)
        );
      }
    }
  }
