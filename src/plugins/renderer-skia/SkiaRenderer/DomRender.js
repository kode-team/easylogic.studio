...
export default class DomRender extends ItemRender {
  ...
  drawBorder(item, renderer) {
    let { CanvasKit, surface, canvas } = renderer;
    ...
    const strokeRect = CanvasKit.XYWHRect(
      halfBorderWidth,
      halfBorderWidth,
      item.screenWidth - borderWidth,
      item.screenHeight - borderWidth
    );
  ...
  renderItem(item, renderer) {
    ...
    const backgroundRect = CanvasKit.XYWHRect(
      0,
      0,
      item.screenWidth,
      item.screenHeight
  ...
  toBackgroundImageCSS(item) {
    ...
    if (item.pattern) {
      ...
      for (var i = 0, len = patternList.length; i < len; i++) {
        list.push.apply(
          list,
          BackgroundImage.parseStyle(STRING_TO_CSS(patternList[i].toCSS()))
        );
      }
    }
  ...
  toDefString(item) {
    ...
    return str
      ? /*html*/ `
    <svg class='inner-svg-element' data-id="${this.innerSVGId(
      item
    )}" width="0" height="0">
      <defs>
        ${str}
