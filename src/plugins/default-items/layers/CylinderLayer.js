...
export class CylinderLayer extends Component {
  ...
  setCustomKeyframes(keyframes, customProperty) {
    if (customProperty.property === "rate") {
      ...
      var faceList = repeat(count).map((it, index) => {
        ...
        return {
          ...
          properties: [
            {
              keyframes: customProperty.keyframes.map(
                ({ time, value, timing }) => {
                  return {
                    time,
                    value: `rotateY(${rotateY}deg) translateZ(${r * value}px)`,
                    timing,
                  };
                }
              ),
    ...
    if (customProperty.property.includes(".color")) {
      var [, index] = customProperty.property.split(".");
  ...
  toNestedCSS() {
    ...
    var faceList = repeat(count).map((it, index) => {
      ...
      return {
        selector: `.face[data-index="${index}"]`,
        cssText: `
  ...
  updateFunction(currentElement) {
    ...
    if (count !== this.json.count.value) {
      ...
      var string = repeat(this.json.count.value)
        .map((it, index) => {
  ...
  get svg() {
    ...
    var common = nestedCSS.find((it) => it.selector === ".face") || {
      cssText: "",
    };
