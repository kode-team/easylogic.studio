...
export default class SVGItemRender extends SVGLayerRender {
  ...
  toFillSVG(item) {
    return SVGFill.parseImage(item.fill || "transparent").toSVGString(
      this.fillId(item)
    );
  }

  toStrokeSVG(item) {
    return SVGFill.parseImage(item.stroke || "black").toSVGString(
      this.strokeId(item)
    );
  }

  toFillValue(item) {
    return SVGFill.parseImage(item.fill || "transparent").toFillValue(
      this.fillId(item)
    );
  }
  ...
  toStrokeValue(item) {
    return SVGFill.parseImage(item.stroke || "black").toFillValue(
      this.strokeId(item)
    );
  }
