...
export default class PathSegmentView extends EditorElement {
    ...
    template() {
        return <div class="elf--path-segment-view">
    ...
    [POINTERSTART('$segmentList .segment') + IF('checkMoveSegment') + MOVE('moveSegment') + END('moveEndSegment')](e) {
    ...
    updatePath() {
        ...
        const target = PathParser.fromStructuredVerties(this.targetPathSegmentList).transformMat4(current.absoluteMatrixInverse);

        this.command("setAttributeForMulti", "update path", this.$selection.packByValue(
            current.updatePath(target.d)
        ))  
    }
    ...
    [LOAD('$segmentList') + DOMDIFF] () {
        ...
        return verties.map((it, index) => {
            return <div class="segment" data-index={index} style={{
    ...
    [LOAD('$guideList') + DOMDIFF] () {
        ...
        for (let i = 0; i < verties.length; i++) {
            ...
            if (Segment.isCubicBezierCurve(it.segment)) {
                ...
                if (it.valueIndex === 0) {
                    if (Segment.isQuadraticBezierCurve(it.prevSegment) || Segment.isCubicBezierCurve(it.prevSegment)) {
                        results.push({
                            start: prev.absoluteVertex,
                            end: it.absoluteVertex,
                        })
        ...
        return <svg>
            {results.map(it => {
                return <line class="guide" x1={it.start[0]} y1={it.start[1]} x2={it.end[0]} y2={it.end[1]}></line>
            })}
        </svg>
