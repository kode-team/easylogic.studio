import { CLICK, DRAGSTART, LOAD } from "sapa";
...
export default class CustomAssets extends EditorElement {
    ...
    return /*html*/ `
      <div class='elf--custom-assets'>
        <div class='custom-assets-tools'>
          ...
          <div class='tools'>
            <button type="button" ref='$addCustomAsset'>${iconUse(
              "add"
            )}</button>
          </div>
        </div>
      ...
      return /*html*/ `
        <div class='asset-preview' draggable="true" data-preview-id="${it.id}">
          ...
          <div class='tools'>
            ...
            <div class='buttons'>
              <button type="button" class='remove-asset-preview' title="remove asset" data-preview-id="${
                it.id
              }">${iconUse("remove")}</button>
            </div>
          </div>
        </div>
      `;
  ...
  async [CLICK("$list .remove-asset-preview")](e) {
    if (
      window.confirm(
        this.$i18n("app.confirm.message.artboard.items.removeCustomAsset")
      )
    ) {
