import { CLICK, LOAD, DEBOUNCE, DOMDIFF, SUBSCRIBE } from "sapa";
...
export default class TimelineObjectList extends EditorElement {
  [SUBSCRIBE("refreshCanvas", REFRESH_SELECTION) + DEBOUNCE(1000)]() {
    this.refresh();
  }
  ...
  makeTimelineObjectRow(animation) {
    ...
    return /*html*/ `
        <div class='timeline-object' data-timeline-animation-id="${obj.id}">
            <div class='timeline-object-row layer' data-selected='${selected}' data-layer-id='${
      obj.id
    }' data-property=''>
                <div class='icon'>${icon.chevron_right}</div>
                <div class='title'> ${obj.name}</div>
                <div class='tools'>
                    <button type="button" class='remove-timeline' data-layer-id='${
                      obj.id
                    }'>${icon.remove}</button>
            ...
            ${animation.properties
              .map((property) => {
                var selected = this.$timeline.checkProperty(
                  obj.id,
                  property.property
                );

                return /*html*/ `
                <div class='timeline-object-row layer-property' data-selected='${selected}' data-layer-id='${
                  obj.id
                }' data-property='${property.property}'>
                  <div class='icon'></div>                    
                    <div class='title'>${getPropertyTitle(
                      property.property
                    )}</div>
                    ...
                    <div class='tools'>
                        <button type="button" class='remove' data-layer-id='${
                          obj.id
                        }' data-property='${property.property}'>${
                  icon.remove
                }</button>
                        <button type="button" class='add' data-layer-id='${
                          obj.id
                        }' data-property='${property.property}'></button>
                    </div>
                </div>`;
              })
              .join("")}                                                      
  ...
  refreshSelection() {
    ...
    this.$timeline.each((it) => {
      var $el = this.$el.$(
        `[data-layer-id="${it.layerId}"][data-property="${it.property}"]`
      );
  ...
  [CLICK("$el .timeline-object-row.layer .title")](e) {
    ...
    this.$timeline.toggleLayerContainer(
      container.attr("data-timeline-animation-id")
    );

    this.refreshSelection();
    this.emit(
      "toggleTimelineObjectRow",
      layerId,
  ...
  [CLICK("$el .timeline-object-row.layer-property .title")](e) {
    var [layerId, property] = e.$dt
      .closest("timeline-object-row")
      .attrs("data-layer-id", "data-property");
