...
export default class SVGFilterEditor extends EditorElement {
    ...
    <div class='filter-item-list' ref="$filterTemplateSelect">
      ...
      ${mapjoin(SVGFilterTemplateList, (f) => {
        return /*html*/ `
          <div class='group' label="${this.$i18n(f.label)}">
            ${mapjoin(f.items, (i) => {
              return /*html*/ ` <div class='item' draggable="true" value="${
                i.value
              }">${this.$i18n(i.label)}</div>`;
            })}
          </div>
        `;
      })}
  ...
  [LOAD("$filterList")]() {
    if (
      ...
      this.state.selectedFilter.isSource() === false
    ) {
  ...
  makeConnectedPath(points) {
    ...
    manager
      .M(first)
      .C(
        { x: first.x + dist, y: first.y },
        { x: last.x - dist, y: last.y },
        last
      );
    ...
    if (this.pointType === "in" || this.pointType === "out") {
      ...
      if (this.pointType === "out") {
        if ($target.hasClass("in")) {
          var targetFilter =
            this.state.filters[+$targetNode.attr("data-index")];
          if (targetFilter) {
  ...
  makeGraphPanel() {
    return this.state.filters.map((it, index) => {
      ...
      return /*html*/ `
        <div class='filter-node ${selectedClass}' data-type="${
        it.type
      }" data-index="${index}" data-filter-id="${it.id}" style='left: ${
        it.bound.x
      }px;top: ${it.bound.y}px;'>
          <div class='label'>${this.$i18n(it.type)}</div>
          <div class='remove'>${iconUse("close")}</div>
          <div class='preview' data-source-type="${getSourceTypeString(
            it.type
          )}" data-filter-type='${it.type}'>${getIcon(it.type)}</div>
          <div class='in-list'>
            ${repeat(it.getInCount())
              .map((itIn, inIndex) => {
          ...
          ${
            it.hasLight()
              ? /*html*/ `<div class='light'  data-index="0"></div>`
              : ""
          }
        </div>
      `;
